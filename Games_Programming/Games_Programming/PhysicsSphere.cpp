...
void PhysicsSphere::Update() {
	...
	_displacement -= ((_initialVelocity * timeDif) + (0.5 * gravity * (timeDif*timeDif))) / 100000;

	//v=u+at
	_finalVelocity = _initialVelocity + (gravity * timeDif);
	_initialVelocity = _finalVelocity;

	std::cout << _displacement << "\n";

	oldTime = newTime;
}


float PhysicsSphere::getDisplacement() {
	return _displacement;
}


void PhysicsSphere::LoadObj() {
...
void PhysicsSphere::checkErrorShader(GLuint shader) {
	// Get log length
	GLint maxLength;
